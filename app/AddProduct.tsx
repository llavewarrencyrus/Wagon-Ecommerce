...
const AddProduct = () => {
    ...
    const [selectedCompleteColors, setSelectedCompleteColors] = useState<{ id: string, color: string, image: string | null, base64: string | null | undefined }[]>([]);
    const [selectedCompleteSizes, setSelectedCompleteSizes] = useState<{ id: string, size: string, dimension: string | null }[]>([]);
    const [variant, setVariant] = useState<{ variant_id: string, color_id: string, color: string, size_id: string, size: string, quantity: number }[]>([]);
    ...
    const toggleColorSelection = (
        color: string, setColorState: React.Dispatch<React.SetStateAction<string[]>>,
        setState: React.Dispatch<React.SetStateAction<{ id: string; color: string; image: string | null, base64: string | null | undefined }[]>>, colorState: string[],
        ...
        setColorState((prev) =>
            prev.includes(color) ? prev.filter(i => i !== color) : [...prev, color]
        );
    ...
    const toggleSizeSelection = (
        size: string, setSizeState: React.Dispatch<React.SetStateAction<string[]>>,
        setState: React.Dispatch<React.SetStateAction<{ id: string; size: string; dimension: string | null }[]>>, sizeState: string[],
        ...
        if (step === 2) {
            ...
            selectedCompleteColors.forEach((color) => {
                selectedCompleteSizes.forEach((size) => {
                    ...
                    const existingVariant = variant.find(variant => variant.color_id === color.id && variant.size_id === size.id);
                    if (!existingVariant) {
                        const id = String(uuid.v4());
                        const newVariant = {
                            variant_id: id,
    ...
    const addProducts = async () => {
        if (!nameState || !priceState || selectedCategories.length === 0 || selectedColors.length === 0 || selectedSizes.length === 0 || variant.length === 0 || selectedMaterials.length === 0 || !descriptionState || imageUris.length === 0) {
        ...
        if (!result.canceled && result.assets && result.assets.length > 0) {
            ...
            setSelectedCompleteColors((prevItems) => {
                ...
                if (existingItemIndex !== -1) {
                    ...
                    return [
                        ...prevItems,
                        { id: String(uuid.v4()), color: color, image: imgUri, base64: base64Data },
                    ];
                }
            });
    ...
    const handleDimensionChange = (id: string, newDimension: string) => {
        setSelectedCompleteSizes((prevSizes) =>
            prevSizes.map((item) =>
                item.id === id ? { ...item, dimension: newDimension } : item
            )
        );
    };
            ...
            {step === 1 && (
                <>
                    <ScrollView>
                        ...
                        <TouchableOpacity onPress={handleImageSelect} style={styles.imgButton}>
                ...
                <>
                    <ScrollView>
                        ...
                        <View style={styles.multiSelectContainer}>
                            <ScrollView horizontal>
                                {colors.map((color) => (
                                    <TouchableOpacity
                                        key={color}
                                        onPress={() => toggleColorSelection(color, setSelectedColors, setSelectedCompleteColors, selectedColors, selectedCompleteColors)}
                        ...
                        <View style={styles.materialList}>
                            {selectedCompleteColors ? (selectedCompleteColors.map((item, index) => (
                                <View key={index} style={{ width: '48%', backgroundColor: '#fff', padding: 15, flexDirection: 'row', marginRight: 'auto', marginBottom: 10 }}>
                                        ...
                                        {item.image ? (
                                            ...
                                            <View>
                                                <View style={{ borderWidth: 1, width: 100, height: (100 * 4) / 3 }}>
                                                    <MaterialCommunityIcons name="file-image-plus-outline" size={63} color='#000' style={{ margin: 'auto' }} />
                                                </View>
                                            </View>
                                        )}
                                    </TouchableOpacity>
                        ...
                        <View style={styles.multiSelectContainer}>
                            <ScrollView horizontal>
                                {sizes.map((size) => (
                                    <TouchableOpacity
                                        key={size}
                                        onPress={() => toggleSizeSelection(size, setSelectedSizes, setSelectedCompleteSizes, selectedSizes, selectedCompleteSizes)}
                        ...
                        <View style={styles.materialList}>
                            {selectedCompleteSizes ? (selectedCompleteSizes.map((item, index) => (
                                <View key={item.id} style={{ marginBottom: 10, flexDirection: 'row', width: '100%' }}>
                                    <Text style={{ textAlignVertical: "center" }}>{item.size}:</Text>
                                    <TextInput
                                        placeholder="Enter dimension"
                                        value={item.dimension ?? ''}
                                        onChangeText={(text) => handleDimensionChange(item.id, text)}
                                        style={{ borderWidth: 0.8, padding: 5, marginVertical: 5, borderRadius: 4, width: '90%', marginLeft: 'auto' }}
                                    />
                                </View>
                            ))) : (
                                null
                ...
                <>
                    <ScrollView>
                        ...
                        <View>
                            {variant ? (
                                variant.map((item) => (
                                    <View key={item.variant_id} style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#fff', marginBottom: 15, padding: 10 }}>
                                        <View style={{ flexDirection: 'row', alignItems: 'center', width: '40%' }}>
                                            <View style={{ width: '70%' }}>
                                                <Text style={{ borderWidth: 0.8, padding: 6, borderRadius: 4, color: Colors.border, marginRight: 'auto', borderColor: Colors.border }}>{item.color}</Text>
                                            </View>
                                            <View style={{ width: '30%' }}>
                                                <Text style={{ borderWidth: 0.8, padding: 6, borderRadius: 4, color: Colors.border, marginRight: 'auto', borderColor: Colors.border }}>{item.size}</Text>
                                            </View>
                                        </View>
